...
import org.multicoder.mcpaintball.entity.BluePaintballHeavyArrowEntity;
import org.multicoder.mcpaintball.init.soundinit;
import org.multicoder.mcpaintball.util.config.MCPaintballConfig;

import java.util.List;

public class BlueBazookaItem extends Item
{

    public BlueBazookaItem()
    {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand)
    {
        if(!level.isClientSide)
        {
            player.getCapability(PlayerTeamCapabilityProvider.CAPABILITY).ifPresent(cap ->
            {
                ...
                if (Team == 3)
                {
                    AbstractArrow AAE = new BluePaintballHeavyArrowEntity(level,player);
                    AAE.shootFromRotation(player, player.getXRot(), player.getYRot(), 0.0f, 5.0f, MCPaintballConfig.BAZOOKA_INACCURACY.get().floatValue());
    ...
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, TooltipFlag pIsAdvanced)
    {
        ...
        {
            pTooltipComponents.add(Component.translatable("text.mcpaintball.bazooka_spec").withStyle(ChatFormatting.BOLD));
        }
        else
        {
